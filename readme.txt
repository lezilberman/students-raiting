...
each student from the stream of students' grades received by them from the beginning of their studies.
Application should run as Docker containers. The results per student Id must be available through the REST API endpoint.

2. Project structure
====================
The Kafka Streams students-raiting project consists of two modules:
...
2) The range of Id generation can be so large (1000 by default),
...
||| STUDENT |||  id =  709   avg.grade =   76
||| STUDENT |||  id =  841   avg.grade =   51
